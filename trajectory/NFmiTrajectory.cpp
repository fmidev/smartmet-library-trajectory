...
NFmiTrajectory::NFmiTrajectory(const NFmiPoint &theLatLon,
...
void NFmiTrajectory::CalculateCrossSectionTrajectoryHelpData(void)
{
  ...
  const std::vector<NFmiPoint> &points = itsMainTrajector.Points();
...
{  // toivottavasti olet poistanut kommentit luettavasta streamista!!
  ...
  NFmiDataStoringHelpers::ReadTimeWithOffsets(
      usedViewMacroTime, itsOriginTime, is);  // Tätä ei ehkä voi oikeasti käyttää!!!

  is >> itsProducer;

  is >> itsDataType >> itsTimeStepInMinutes >> itsTimeLengthInHours >> itsPlumeProbFactor >>
      itsPlumeParticleCount;

  is >> itsStartLocationRangeInKM >> itsPressureLevel >> itsStartPressureLevelRange >>
