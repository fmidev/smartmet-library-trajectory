...
class NFmiTempBalloonTrajectorSettings
{
 public:
  ...
  void CurrentFloatTimeInMinutes(double newValue)
  {
    itsCurrentFloatTimeInMinutes = newValue;
  }  // tällä lähinnä nollataan kellunta aika
  void AddFloatTime(double theAddedTimeInMinutes)
  {
    itsCurrentFloatTimeInMinutes += theAddedTimeInMinutes;
  }
  void Reset(void)
  {
 ...
 private:
  ...
  double itsCurrentFloatTimeInMinutes;  // jos 0, ei kellunta ole vielä alkanut, tämän avulla
                                        // lasketaan milloin kellunta lopetetaan
};

inline std::ostream &operator<<(std::ostream &os, const NFmiTempBalloonTrajectorSettings &item)
{
  item.Write(os);
  return os;
}
inline std::istream &operator>>(std::istream &is, NFmiTempBalloonTrajectorSettings &item)
{
  item.Read(is);
  return is;
}
