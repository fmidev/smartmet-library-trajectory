...
void list_formats()
{
  ...
  std::cerr << "Formats found from template directory '" << options.templatedir << "':" << std::endl << std::endl;

  size_t count = 0;
  for(fs::directory_iterator i(p), end; i!=end; ++i)
	{
	  if(i->path().extension() == ".c2t")
		std::cerr << std::setw(3) << std::right << ++count << " " << i->path().stem().string() << std::endl;
	}

  std::cerr << std::endl << "Found " << count << (count!=1 ? " formats." : " format.") << std::endl;
}
